...
public enum Message implements MessageResource {
    ...
    DOMA4055("戻り値の型[{0}]とIterationCallbackの2番目の実型引数の型[{1}]が一致していません。"),
    DOMA4056("@Selectのload要素にLoadType.ITERATIONを設定した場合、IterationCallback型のパラメータが必要です。"),
    DOMA4057("IterationCallback型のパラメータを利用するには、load要素にLoadType.ITERATIONを設定しなければいけません。"),
    DOMA4058("IterationCallbackの1番目の実型引数の型は、基本型、ドメインクラス、エンティティクラス、java.util.Map<String, Object>のいずれかでなければいけません。"),
    ...
    DOMA4093("@Versionは次の型のプロパティに対してのみ有効です。数値型、数値型のドメインクラス、もしくはそれらを要素とするOptional型。"),
    DOMA4095("@GeneratedValueは次の型のプロパティに対してのみ有効です。数値型、数値型のドメインクラス、もしくはそれらを要素とするOptional型"),
    ...
    DOMA4158("IterationCallbackの1番目の実型引数に抽象型のエンティティクラス[{0}]は指定できません。"),
    ...
    DOMA4214("クラス[{0}]の原型はIterationCallbackの2番目の実型引数に使用できません。"),
    DOMA4215("クラス[{0}]の型引数にワイルドカードや型パラメータを含むものはIterationCallbackの2番目の実型引数に使用できません。"),
    DOMA4216("クラス[{0}]の原型はIterationCallbackの1番目の実型引数に使用できません。"),
    DOMA4217("クラス[{0}]の型引数にワイルドカードや型パラメータを含むものはIterationCallbackの1番目の実型引数に使用できません。"),
    ...
    DOMA4245("Streamの実型引数の型は、基本型、ドメインクラス、エンティティクラス、java.util.Map<String, Object>のいずれかでなければいけません。"),
    DOMA4246("戻り値の型[{0}]とFunctionの2番目の実型引数の型[{1}]が一致していません。"),
    DOMA4247("@Selectのload要素にLoadType.STREAMを設定した場合、Function型のパラメータが必要です。"),
