...
public enum Message implements MessageResource {
  ...
  DOMA4030(
      "@SequenceGeneratorを使用する場合、同じフィールドに@GeneratedValue(strategy = GenerationType.SEQUENCE)を指定しなければいけません。 at {0}.{1}"),
  DOMA4031(
      "@TableGeneratorを使用する場合、同じフィールドに@GeneratedValue(strategy = GenerationType.TABLE)を指定しなければいけません。 at {0}.{1}"),
  DOMA4033("@GeneratedValueを使用する場合、同じフィールドに@Idを指定しなければいけません。 at {0}.{1}"),
  DOMA4034(
      "@GeneratedValueのstrategy要素にGenerationType.SEQUECNEを指定する場合、@SequenceGeneratorの指定も必要です。 at {0}.{1}"),
  DOMA4035(
      "@GeneratedValueのstrategy要素にGenerationType.TABLEを指定する場合、@TableGeneratorの指定も必要です。 at {0}.{1}"),
  ...
  DOMA4066(
      "@Functionもしくは@Procedureが注釈されたメソッドのパラメータには、@In、@InOut、@Out、@ResultSetのいずれかの注釈が必須です。 at {0}.{1}"),
  ...
  DOMA4096(
      "クラス[{0}]は、永続対象の型としてサポートされていません。 at {1}.{2}。@ExternalDomainでマッピングすることを意図している場合、登録や設定が不足している可能性があります。@DomainConvertersを注釈したクラスと注釈処理のオプション（doma.domain.converters）を見直してください。"),
  ...
  DOMA4103(
      "型[{0}]をパラメータにもつ非privateなコンストラクタが見つかりません。コンストラクタを定義するか、ファクトリメソッドを利用したい場合は@DomainのfactoryMethod属性にメソッド名を指定してください。 at {1}"),
  DOMA4104(
      "アクセッサメソッド[{0}]が見つかりません。アクセッサメソッドは、型[{1}]を戻り値とする非privateで引数なしのインスタンスメソッドでなければいけません。 at {2}"),
  DOMA4105("クラスまたはインタフェースまたは列挙型以外には注釈できません。 at {0}"),
  DOMA4106(
      "ファクトリメソッド[{0}]が見つかりません。ファクトリメソッドは、型[{1}]を戻り値とし型[{2}]をパラメータにもつ、非privateなstaticメソッドでなければいけません。また、メソッドの型パラメータはクラスの型パラメータと同一でなければいけません。ファクトリメソッドを定義するか、ファクトリメソッドが不要な場合は@DomainのfactoryMethodに\"new\"と指定しコンストラクタを定義してください。 at {3}"),
  ...
  DOMA4124(
      "ミュータブルなエンティティクラスは非privateな引数なしのコンストラクタを持たねばなりません。イミュータブルにするには@Entityのimmutable要素にtrueを設定してください。 at {0}"),
  ...
  DOMA4153(
      "SQLファイル[{0}]の妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。バインド変数もしくはリテラル変数[{4}]に対応するパラメータの型は基本型もしくはドメインクラスでなければいけません。しかし、実際の型は[{5}]です。型を間違えていませんか？もしくはフィールドアクセスやメソッド呼び出しの記述を忘れていませんか？SQL[{1}]"),
  ...
  DOMA4161(
      "SQLファイル[{0}]の妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。括弧の前に位置するバインド変数もしくはリテラル変数[{4}]に対応するパラメータの型は基本型もしくはドメインクラスを要素としたjava.lang.Iterableのサブタイプでなければいけません。しかし、実際の型は[{5}]です。型を間違えていませんか？もしくはフィールドアクセスやメソッド呼び出しの記述を忘れていませんか？。SQL[{1}]"),
  ...
  DOMA4184(
      "列挙型に@Domainを注釈する場合、factoryMethod属性に\"new\"は指定できません（\"new\"はコンストラクタで生成することを示します）。staticで非privateなファクトリメソッドの名前を指定してください。 at {0}"),
  DOMA4185(" ... /** SQLが長すぎるため最初の{0}文字のみを表示しています。 */"),
  DOMA4186(
      "java.util.Listに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ドメインクラス、エンティティクラス、java.util.Map<String, Object>。 at {1}.{2}"),
  DOMA4188(
      "継承先に指定できる「@Daoが注釈されたインタフェース」は1つのみです。@Daoが注釈されておらず全てのメソッドがデフォルトメソッドであるインタフェースは複数指定できます。 at {0}"),
  DOMA4189(
  ...
  DOMA4193(
      "@ExternalDomainを注釈したクラスは引数なしのpublicなコンストラクタを持たなければいけません。クラス[{0}]には引数なしのpublicなコンストラクタが見つかりません。"),
  DOMA4194(
      "org.seasar.doma.jdbc.domain.DomainConverterの第2型引数に指定された型[{0}]は永続対象としてサポートされていません。 at {1}"),
  DOMA4196("型[{0}]が@ExternalDomainで注釈されていません。"),
  DOMA4197(
      "org.seasar.doma.jdbc.domain.DomainConverterの第1型引数に指定された型[{0}]のパッケージがデフォルトパッケージです。サポートされていません。 at {1}"),
  ...
  DOMA4203(
      "org.seasar.doma.jdbc.domain.DomainConverterの第1型引数に指定された型[{0}]への型引数はすべてワイルドカードでなければいけません。 at {1}"),
  ...
  DOMA4222(
      "イミュータブルなエンティティクラスを@Insertや@Updateや@Deleteが注釈されたメソッドのパラメータとする場合、戻り値はorg.seasar.doma.jdbc.Result<E>でなければいけません。型パラメータ E の実型引数にはパラメータと同じエンティティクラスを指定してください。 at {0}.{1}"),
  DOMA4223(
      "イミュータブルなエンティティクラスを@BatchInsertや@BatchUpdateや@BatchDeleteが注釈されたメソッドのパラメータとする場合、戻り値はorg.seasar.doma.jdbc.BatchResult<E>でなければいけません。型パラメータEの実型引数にはパラメータと同じエンティティクラスを指定してください。 at {0}.{1}"),
  ...
  DOMA4228(
      "エンティティリスナークラスの型パラメータ[{0}]がクラス階層の中でorg.seasar.doma.jdbc.entity.EntityListerの型引数として渡されていません。 at {1}"),
  ...
  DOMA4231(
      "親エンティティクラスから引き継がれたエンティティリスナークラス[{0}]の型パラメータ[{1}]の上限境界[{2}]がエンティティクラス[{3}]と互換性がありません。 at {3}"),
  ...
  DOMA4235(
      "java.util.Optionalに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ドメインクラス、エンティティクラス。  at {1}.{2}"),
  ...
  DOMA4254(
      "メソッド[{0}]が見つかりません。メソッドの修飾子はpublic static、戻り値の型はこのクラス[{1}]、パラメータの数は0、でなければいけません。 at {1}"),
  ...
  DOMA4266(
      "@Selectのstrategy要素にSelectStrategyType.COLLECTを設定した場合、Collector型のパラメータが必要です。 at {0}.{1}"),
  DOMA4267(
      "java.util.List内のjava.util.Optionalに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ドメインクラス。 at {1}.{2}"),
  DOMA4268(
      "インターフェースに@Domainを注釈する場合、factoryMethod属性に\"new\"は指定できません（\"new\"はコンストラクタで生成することを示します）。staticで非privateなファクトリメソッドの名前を指定してください。 at {0}"),
  DOMA4270(
  ...
  DOMA4273(
      "java.util.stream.Stream内のjava.util.Optionalに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ドメインクラス。"),
  /** Streamをアプリケーションでクローズしなければいけないことを示す警告メッセージ */
  DOMA4274(
      "java.util.stream.StreamがDaoメソッドから返されます。アプリケーションで必ずStreamをクローズしてください。この警告を抑制するには、メソッドに@Suppress(messages = '{ Message.DOMA4274 }')と注釈してください。 at {0}.{1}"),
  ...
  DOMA4278(
      "型[{0}]がpublicかつstaticでありません。DomainConverterの第1型引数に指定された型もしくはその型を囲む型はpublicかつstaticでなければいけません。"),
  DOMA4279(
      "型[{0}]がローカルクラスもしくは無名クラスです。DomainConverterの第1型引数に指定された型もしくはその型を囲む型はトップレベルもしくはメンバでなければいけません。"),
  DOMA4280(
      "型[{0}]の単純名に\"$\"または\"__\"が含まれています。DomainConverterの第1型引数に指定された型もしくはその型を囲む型はこれらの文字列を単純名に含んではいけません。"),
  ...
  DOMA4422(
      "@lombok.AllArgsConstructorのaccess要素にlombok.AccessLevel.PRIVATEを設定することはサポートされていません。 at {0}"),
  DOMA4423(DOMA4419.getMessagePattern()),
  DOMA4424(DOMA4421.getMessagePattern()),
  DOMA4425(DOMA4422.getMessagePattern()),
  DOMA4426(
      "@lombok.AllArgsConstructorのaccess要素にlombok.AccessLevel.NONEを設定することはサポートされていません。 at {0}"),
  ...
  DOMA4432(
      "@lombok.Valueによって初期化されるインスタンスフィールドの型[{0}]が@DomainのvalueType要素に指定された型[{1}]と一致しません。 at {2}.{3}"),
  ...
  DOMA4440(
      "継承先のメソッド[{0}]がデフォルトメソッドではありません。@Daoが注釈されていないインタフェースをextendsする場合、そのインタフェースの継承先を含む全てのメソッドはデフォルトメソッドでなければいけません。 at {1}"),
