...
public enum Message implements MessageResource {
    ...
    DOMA4088("@Idもしくは@Versionを注釈する場合、@Columnのinsertableにfalseを指定してはいけません。 at {0}.{1}"),
    DOMA4089("@Idもしくは@Versionを注釈する場合、@Columnのupdatableにfalseを指定してはいけません。 at {0}.{1}"),
    ...
    DOMA4096("クラス[{0}]は、永続対象の型としてサポートされていません。 at {1}.{2}。@ExternalHolderでマッピングすることを意図している場合、登録や設定が不足している可能性があります。@HolderConvertersを注釈したクラスと注釈処理のオプション（doma.holder.converters）を見直してください。"),
    ...
    DOMA4103("型[{0}]をパラメータにもつ非privateなコンストラクタが見つかりません。コンストラクタを定義するか、ファクトリメソッドを利用したい場合は@HolderのfactoryMethod属性にメソッド名を指定してください。 at {1}"),
    ...
    DOMA4106("ファクトリメソッド[{0}]が見つかりません。ファクトリメソッドは、型[{1}]を戻り値とし型[{2}]をパラメータにもつ、非privateなstaticメソッドでなければいけません。また、メソッドの型パラメータはクラスの型パラメータと同一でなければいけません。ファクトリメソッドを定義するか、ファクトリメソッドが不要な場合は@HolderのfactoryMethodに\"new\"と指定しコンストラクタを定義してください。 at {3}"),
    ...
    DOMA4132("@HolderのfactoryMethod要素の値が\"new\"の場合、クラスはabstractであってはいけません。 at {0}"),
    ...
    DOMA4184("列挙型に@Holderを注釈する場合、factoryMethod属性に\"new\"は指定できません（\"new\"はコンストラクタで生成することを示します）。staticで非privateなファクトリメソッドの名前を指定してください。 at {0}"),
    ...
    DOMA4191("@ExternalHolderはorg.seasar.doma.jdbc.holder.HolderConverterのサブタイプにのみ注釈できます。 at {0}"),
    ...
    DOMA4194("org.seasar.doma.jdbc.holder.HolderConverterの第2型引数に指定された型[{0}]は永続対象としてサポートされていません。 at {1}"),
    DOMA4196("型[{0}]が@ExternalHolderで注釈されていません。"),
    DOMA4197("org.seasar.doma.jdbc.holder.HolderConverterの第1型引数に指定された型[{0}]のパッケージがデフォルトパッケージです。サポートされていません。 at {1}"),
    DOMA4198("@ExternalHolderが注釈されたクラスはトップレベルでなければいけません。 at {0}"),
    ...
    DOMA4203("org.seasar.doma.jdbc.holder.HolderConverterの第1型引数に指定された型[{0}]への型引数はすべてワイルドカードでなければいけません。 at {1}"),
    ...
    DOMA4268("インターフェースに@Holderを注釈する場合、factoryMethod属性に\"new\"は指定できません（\"new\"はコンストラクタで生成することを示します）。staticで非privateなファクトリメソッドの名前を指定してください。 at {0}"),
    ...
    DOMA4298("クラス[{0}]は、永続対象の型としてサポートされていません。 at {1}.{2}。@ExternalHolderでマッピングすることを意図している場合、登録や設定が不足している可能性があります。@HolderConvertersを注釈したクラスと注釈処理のオプション（doma.holder.converters）を見直してください。"),
    ...
    DOMA4429("@lombok.Valueによって生成されるメソッド名[{0}]が@HolderのaccessorMethod要素に指定された値[{1}]と一致しません。 at {2}"),
    ...
    DOMA4432("@lombok.Valueによって初期化されるインスタンスフィールドの型[{0}]が@HolderのvalueType要素に指定された型[{1}]と一致しません。 at {2}.{3}"),
