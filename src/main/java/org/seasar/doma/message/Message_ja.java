...
public enum Message_ja implements MessageResource {
    ...
    DOMA4096("クラス[{0}]は、永続対象の型としてサポートされていません。@ExternalHolderでマッピングすることを意図している場合、登録や設定が不足している可能性があります。@HolderConvertersを注釈したクラスと注釈処理のオプション（doma.holder.converters）を見直してください。"),
    ...
    DOMA4103("型[{0}]をパラメータにもつ非privateなコンストラクタが見つかりません。コンストラクタを定義するか、ファクトリメソッドを利用したい場合は@HolderのfactoryMethod属性にメソッド名を指定してください。"),
    ...
    DOMA4106("ファクトリメソッド[{0}]が見つかりません。ファクトリメソッドは、型[{1}]を戻り値とし型[{2}]をパラメータにもつ、非privateなstaticメソッドでなければいけません。また、メソッドの型パラメータはクラスの型パラメータと同一でなければいけません。ファクトリメソッドを定義するか、ファクトリメソッドが不要な場合は@HolderのfactoryMethodに\"new\"と指定しコンストラクタを定義してください。"),
    ...
    DOMA4153("SQLファイル[{0}]の妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。バインド変数もしくはリテラル変数[{4}]に対応するパラメータの型は基本型もしくはホルダークラスでなければいけません。しかし、実際の型は[{5}]です。型を間違えていませんか？もしくはフィールドアクセスやメソッド呼び出しの記述を忘れていませんか？SQL[{1}]"),
    ...
    DOMA4161("SQLファイル[{0}]の妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。括弧の前に位置するバインド変数もしくはリテラル変数[{4}]に対応するパラメータの型は基本型もしくはホルダークラスを要素としたjava.lang.Iterableのサブタイプでなければいけません。しかし、実際の型は[{5}]です。型を間違えていませんか？もしくはフィールドアクセスやメソッド呼び出しの記述を忘れていませんか？。SQL[{1}]"),
    ...
    DOMA4184("列挙型に@Holderを注釈する場合、factoryMethod属性に\"new\"は指定できません（\"new\"はコンストラクタで生成することを示します）。staticで非privateなファクトリメソッドの名前を指定してください。"),
    DOMA4185(" ... /** SQLが長すぎるため最初の{0}文字のみを表示しています。 */"),
    DOMA4186("java.util.Listに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ホルダークラス、エンティティクラス、java.util.Map<String, Object>。"),
    DOMA4188("継承先に指定できる「@Daoが注釈されたインタフェース」は1つのみです。@Daoが注釈されておらず全てのメソッドがデフォルトメソッドであるインタフェースは複数指定できます。"),
    ...
    DOMA4193("@ExternalHolderを注釈したクラスは引数なしのpublicなコンストラクタを持たなければいけません。クラス[{0}]には引数なしのpublicなコンストラクタが見つかりません。"),
    ...
    DOMA4197("org.seasar.doma.jdbc.holder.HolderConverterの第1型引数に指定された型[{0}]のパッケージがデフォルトパッケージです。サポートされていません。"),
    ...
    DOMA4203("org.seasar.doma.jdbc.holder.HolderConverterの第1型引数に指定された型[{0}]への型引数はすべてワイルドカードでなければいけません。"),
    ...
    DOMA4235("java.util.Optionalに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ホルダークラス、エンティティクラス。"),
    ...
    DOMA4267("java.util.List内のjava.util.Optionalに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ホルダークラス。"),
    DOMA4268("インターフェースに@Holderを注釈する場合、factoryMethod属性に\"new\"は指定できません（\"new\"はコンストラクタで生成することを示します）。staticで非privateなファクトリメソッドの名前を指定してください。"),
    ...
    DOMA4273("java.util.stream.Stream内のjava.util.Optionalに対する実型引数の型[{0}]はサポートされていません。サポートされている型は次のものです。基本型、ホルダークラス。"),
    ...
    DOMA4278("型[{0}]がpublicかつstaticでありません。HolderConverterの第1型引数に指定された型もしくはその型を囲む型はpublicかつstaticでなければいけません。"),
    DOMA4279("型[{0}]がローカルクラスもしくは無名クラスです。HolderConverterの第1型引数に指定された型もしくはその型を囲む型はトップレベルもしくはメンバでなければいけません。"),
    DOMA4280("型[{0}]の単純名に\"$\"または\"__\"が含まれています。HolderConverterの第1型引数に指定された型もしくはその型を囲む型はこれらの文字列を単純名に含んではいけません。"),
    ...
    DOMA4298("クラス[{0}]は、永続対象の型としてサポートされていません。。@ExternalHolderでマッピングすることを意図している場合、登録や設定が不足している可能性があります。@HolderConvertersを注釈したクラスと注釈処理のオプション（doma.holder.converters）を見直してください。"),
    ...
    DOMA4440("継承先のメソッド[{0}]がデフォルトメソッドではありません。@Daoが注釈されていないインタフェースをextendsする場合、そのインタフェースの継承先を含む全てのメソッドはデフォルトメソッドでなければいけません。"),
