...
public enum Message_ja implements MessageResource {
  ...
  DOMA4069("SQLテンプレートの解析に失敗しました。PATH=[{0}]。原因は次のものです。{1}"),
  DOMA4071(
  ...
  DOMA4092("SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。詳細は次のものです。{4} SQL[{1}]。"),
  ...
  DOMA4122("SQLテンプレートの妥当検査に失敗しました。{0} メソッドのパラメータ[{1}]がSQLテンプレートで参照されていません。"),
  DOMA4124(
  ...
  DOMA4140(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。/*%if ...*/の式[{4}]が型[{5}]として評価されましたが、boolean/Boolean型でなければいけません。SQL[{1}]"),
  DOMA4141(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。/*%elseif ...*/の式[{4}]が型[{5}]として評価されましたが、boolean/Boolean型でなければいけません。SQL[{1}]"),
  DOMA4143("ファイル[{0}]の取得ができません。原因は次のものです。{1}"),
  ...
  DOMA4149(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。/*%for ...*/の式[{4}]が型[{5}]として評価されましたが、java.lang.Iterable型でなければいけません。SQL[{1}]"),
  DOMA4150(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。/*%for ...*/の式[{4}]の型[{5}]の実型引数が不明です。SQL[{1}]"),
  DOMA4153(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。バインド変数もしくはリテラル変数[{4}]に対応するパラメータの型は基本型もしくはホルダークラスでなければいけません。しかし、実際の型は[{5}]です。型を間違えていませんか？もしくはフィールドアクセスやメソッド呼び出しの記述を忘れていませんか？SQL[{1}]"),
  ...
  DOMA4161(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。括弧の前に位置するバインド変数もしくはリテラル変数[{4}]に対応するパラメータの型は基本型もしくはホルダークラスを要素としたjava.lang.Iterableのサブタイプでなければいけません。しかし、実際の型は[{5}]です。型を間違えていませんか？もしくはフィールドアクセスやメソッド呼び出しの記述を忘れていませんか？。SQL[{1}]"),
  ...
  DOMA4181(
      "SQLテンプレートに埋め込み変数コメントが含まれています。PATH=[{0}]。バッチの中で実行されるSQLは一定であるため、埋め込み変数コメントにより動的なSQLを発行しようとしても意図したSQLにならない可能性があります。この警告を抑制するには、メソッドに@Suppress(messages = '{ Message.DOMA4181 }')と注釈してください。"),
  /** SQLファイルに条件コメントが含まれていることを示す警告メッセージ */
  DOMA4182(
      "SQLテンプレートに条件コメントが含まれています。PATH=[{0}]。バッチの中で実行されるSQLは一定であるため、条件コメントにより動的なSQLを発行しようとしても意図したSQLにならない可能性があります。この警告を抑制するには、メソッドに@Suppress(messages = '{ Message.DOMA4182 }')と注釈してください。"),
  /** SQLファイルに繰り返しコメントが含まれていることを示す警告メッセージ */
  DOMA4183(
      "SQLテンプレートに繰り返しコメントが含まれています。PATH=[{0}]。バッチの中で実行されるSQLは一定であるため、繰り返しコメントにより動的なSQLを発行しようとしても意図したSQLにならない可能性があります。この警告を抑制するには、メソッドに@Suppress(messages = '{ Message.DOMA4183 }')と注釈してください。"),
  DOMA4184(
  ...
  DOMA4257(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。/*%expand ...*/の式が使用されていますが、カラムの自動展開ができません。メソッドに@Selectが注釈され、結果セットのレコードがエンティティクラスにマッピングされていることを確認してください。SQL[{1}]"),
  ...
  DOMA4270(
      "SQLテンプレートの妥当検査に失敗しました（[{2}]行目[{3}]番目の文字付近）。PATH=[{0}]。/*%populate */の式が使用されていますが、SET句の自動生成ができません。メソッドに@Updateまたは@BatchUpdateが注釈され、第一引数がエンティティクラスにマッピングされていることを確認してください。SQL[{1}]"),
