...
public class StandardDialect implements Dialect {
  ...
  public SqlNode transformSelectSqlNode(SqlNode sqlNode, SelectOptions options) {
    ...
    var transformed = sqlNode;
    if (SelectOptionsAccessor.isCount(options)) {
      transformed = toCountCalculatingSqlNode(sqlNode);
    }
    var offset = SelectOptionsAccessor.getOffset(options);
    var limit = SelectOptionsAccessor.getLimit(options);
    if (offset >= 0 || limit >= 0) {
      transformed = toPagingSqlNode(transformed, offset, limit);
    }
    var forUpdateType = SelectOptionsAccessor.getForUpdateType(options);
    if (forUpdateType != null) {
      var aliases = SelectOptionsAccessor.getAliases(options);
      ...
      var waitSeconds = SelectOptionsAccessor.getWaitSeconds(options);
  ...
  protected SqlNode toPagingSqlNode(SqlNode sqlNode, long offset, long limit) {
    var transformer = new StandardPagingTransformer(offset, limit);
  ...
  protected SqlNode toForUpdateSqlNode(
    ...
    var transformer = new StandardForUpdateTransformer(forUpdateType, waitSeconds, aliases);
    return transformer.transform(sqlNode);
  }
  ...
  protected SQLException getCauseSQLException(SQLException sqlException) {
    var cause = sqlException;
    for (var t : sqlException) {
  ...
  public static class StandardExpressionFunctions implements ExpressionFunctions {
    ...
    public String prefix(CharSequence text) {
      ...
      var escaped = escapeWildcard(defaultWildcardReplacementPattern, text, defaultReplacement);
      return escaped + "%";
    }
    ...
    public java.util.Date roundDownTimePart(java.util.Date date) {
      ...
      var calendar = makeRoundedDownCalendar(date);
    ...
    public Timestamp roundDownTimePart(Timestamp timestamp) {
      ...
      var calendar = makeRoundedDownCalendar(timestamp);
    ...
    public java.util.Date roundUpTimePart(java.util.Date date) {
      ...
      var calendar = makeRoundedUpCalendar(date);
      return new java.util.Date(calendar.getTimeInMillis());
    }
    ...
    public Timestamp roundUpTimePart(Timestamp timestamp) {
      ...
      var calendar = makeRoundedUpCalendar(timestamp);
      return new Timestamp(calendar.getTimeInMillis());
    }
  ...
  public static class StandardScriptBlockContext implements ScriptBlockContext {
    ...
    protected void check() {
      for (var startKeywords : sqlBlockStartKeywordsList) {
        if (startKeywords.size() > keywords.size()) {
          continue;
        }
        var startKeywordsIt = startKeywords.iterator();
        var keywordsIt = keywords.iterator();
        for (; startKeywordsIt.hasNext(); ) {
          var word1 = startKeywordsIt.next();
          var word2 = keywordsIt.next();
