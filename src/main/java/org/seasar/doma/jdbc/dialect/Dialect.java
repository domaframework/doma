...
public interface Dialect {
   ...
   * @throws JdbcException オプションの指定に対応するSQLの変換がサポートされていない場合
   */
   ...
   * @throws DomaNullPointerException {@code sqlException} が {@code null} の場合
   */
   ...
   *
   * @param type 悲観的排他制御の種別
   * @param withTargets ロックの対象が指定されている場合 {@code true}
   * @return サポートしている場合 {@code true}
   */
  boolean supportsSelectForUpdate(SelectForUpdateType type, boolean withTargets);

  /**
   * ストアドプロシージャ-やストアドファンクションで {@link ResultSet} をOUTパラメータとして戻すことをサポートしてるかどうかを返します。
   *
   * @return サポートしている場合 {@code true}
   */
   ...
   *
   * <p>{@link #supportsIdentity()} が {@code true} を返す場合にのみ呼び出し可能です。
   *
   * @param catalogName カタログの名前
   * @param schemaName スキーマの名前
   * @param tableName テーブルの名前
   * @param columnName IDENTITYカラムの名前
   * @param isQuoteRequired 引用符が必要かどうか
   * @param isIdColumnQuoteRequired ID列に引用符が必要かどうか
   * @return IDENTITYを取得するためのSQL
   * @throws DomaNullPointerException {@code tableName} と {@code columnName} のいずれかが {@code null} の場合
   */
   ...
   *
   * <p>{@link #supportsIdentityReservation()} が {@code true} を返す場合にのみ呼び出し可能です。
   *
   * @param catalogName カタログの名前
   * @param schemaName スキーマの名前
   * @param tableName テーブルの名前
   ...
   *
   * <p>{@link #supportsSequence()} が {@code true} を返す場合にのみ呼び出し可能です。
   *
   * @param qualifiedSequenceName シーケンスの完全修飾名
   * @param allocationSize 割り当てサイズ
   * @return シーケンスの次の値を取得するためのSQL
   * @throws DomaNullPointerException {@code qualifiedSequenceName} が {@code null} の場合
   */
   ...
   *
   * <p>{@link #supportsResultSetReturningAsOutParameter()} が {@code true} を返す場合にのみ呼び出し可能です。
   *
