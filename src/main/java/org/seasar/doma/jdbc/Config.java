...
public interface Config {
   ...
   * データソース名を返します。
   *
   * <p>データソースを複数扱う場合、データソースごとに異なる名前を返さなければいけません。この値は、シーケンスやテーブルを使用した識別子の自動生成機能で、
   ...
   *
   * <p>{@code REQUIRES_NEW}のトランザクション属性を制御するコントローラーは、テーブルを使用した識別子の自動生成機能において、
   ...
   * トランザクションマネジャーを返します。
   *
   * <p>デフォルトの実装では {@link UnsupportedOperationException} をスローします。
   *
   * @return トランザクションマネジャー
   * @throws UnsupportedOperationException {@link TransactionManager} のインタフェースを使ったトランザクションをサポートしない場合
   * @since 2.0.0
   */
   ...
   * 最大行数の制限値を返します。
   *
   * <p>0以下の値は、 {@link Statement#setMaxRows(int)}へは渡されません。
   *
   * @return 最大行数の制限値
   ...
   * フェッチサイズを返します。
   *
   * <p>0以下の値は、 {@link Statement#setFetchSize(int)}へは渡されません。
   *
   * @return フェッチサイズ
   ...
   * クエリタイムアウト（秒）を返します。
   *
   * <p>0以下の値は、 {@link Statement#setQueryTimeout(int)}へは渡されません。
   *
   * @return クエリタイムアウト（秒）
   ...
   * バッチサイズを返します。
   *
   * <p>{@literal 1} 以下の値は、 {@literal 1} とみなされます。
   *
   ...
   *
   * @param provider {@link ConfigProvider} を実装していることを期待されるオブジェクト
   * @return {@link ConfigProvider} が返した {@link Config}
   * @throws DomaIllegalArgumentException {@code provider} が {@link ConfigProvider} でない場合
   * @since 2.0.0
   */
