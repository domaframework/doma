...
public class AutoBatchInsertQuery<ENTITY> extends AutoBatchModifyQuery<ENTITY>
    ...
    protected void preInsert() {
        AutoBatchPreInsertContext<ENTITY> context = new AutoBatchPreInsertContext<>(
        ...
        if (generatedIdPropertyDesc != null) {
            if (idGenerationConfig == null) {
                idGenerationConfig = new IdGenerationConfig(config, entityDesc,
                        new ReservedIdProvider(config, entityDesc, entities.size()));
            ...
            if (propertyDesc.isId()) {
                ...
                if (generatedIdPropertyDesc == null) {
                    ...
                    if (property.getWrapper().get() == null) {
                        throw new JdbcException(Message.DOMA2020, entityDesc.getName(),
    ...
    protected void prepareIdValue() {
        if (generatedIdPropertyDesc != null && idGenerationConfig != null) {
            currentEntity = generatedIdPropertyDesc.preInsert(entityDesc, currentEntity,
                    idGenerationConfig);
        }
    }
    ...
    protected void prepareSql() {
        ...
        PreparedSqlBuilder builder = new PreparedSqlBuilder(config, SqlKind.BATCH_INSERT,
                sqlLogType);
    ...
    public void generateId(Statement statement, int index) {
        if (generatedIdPropertyDesc != null && idGenerationConfig != null) {
            ENTITY newEntity = generatedIdPropertyDesc.postInsert(entityDesc, entities.get(index),
    ...
    protected void postInsert() {
        AutoBatchPostInsertContext<ENTITY> context = new AutoBatchPostInsertContext<>(
                entityDesc, method, config);
