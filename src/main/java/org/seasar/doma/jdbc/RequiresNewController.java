 ...
 *
 * <p>{@literal REQUIRES_NEW} の属性をもつトランザクションの最適な制御方法は、環境ごとに異なります。たとえば、Seasar2.4を利用している場合、
 * 次のような実装がふさわしいでしょう。
 *
 * <h3>実装例</h3>
 *
...
public interface RequiresNewController {
   ...
   *
   * @param <R> 戻り値の型
   * @param callback {@literal REQUIRES_NEW} のトランザクション属性下で扱いたい処理
   * @return 任意の値
   * @throws DomaNullPointerException ｛@code callback} が {@code null} の場合
   * @throws Throwable ｛@code callback} の処理中に何らかの例外が発生した場合
   */
