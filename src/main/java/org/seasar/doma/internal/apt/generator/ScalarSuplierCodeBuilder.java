...
import static org.seasar.doma.internal.apt.generator.CodeHelper.wrapper;
...
class ScalarSuplierCodeBuilder extends SimpleCtTypeVisitor<String, Boolean, RuntimeException> {
    ...
    public String visitHolderCtType(HolderCtType ctType, Boolean p) throws RuntimeException {
        String holderDesc = holderDesc(ctType);
        if (p) {
            return String.format("() -> %1$s.createOptionalScalar()", holderDesc);
        }
        return String.format("() -> %1$s.createScalar()", holderDesc);
    }
}
