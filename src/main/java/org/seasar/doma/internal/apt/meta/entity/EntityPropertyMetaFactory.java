...
public class EntityPropertyMetaFactory {
    ...
    private CtType createCtType() {
        ...
        CtType ctType = ctTypes.toCtType(type,
                List.of(ctTypes::newOptionalCtType, ctTypes::newOptionalIntCtType,
                        ctTypes::newOptionalLongCtType, ctTypes::newOptionalDoubleCtType,
                        ctTypes::newHolderCtType, ctTypes::newEmbeddableCtType,
    ...
    private void doSequenceIdGeneratorMeta(EntityPropertyMeta propertyMeta) {
        SequenceGeneratorReflection sequenceGeneratorReflection = ctx.getReflections()
    ...
    private void validateSequenceIdGenerator(EntityPropertyMeta propertyMeta,
        ...
        TypeElement typeElement = ctx.getTypes()
                .toTypeElement(sequenceGeneratorReflection.getImplementerValue());
        if (typeElement == null) {
            throw new AptIllegalStateException("failed to convert to TypeElement");
        }
        if (typeElement.getModifiers().contains(Modifier.ABSTRACT)) {
            throw new AptException(Message.DOMA4170, fieldElement,
        ...
        if (constructor == null || !constructor.getModifiers().contains(Modifier.PUBLIC)) {
            throw new AptException(Message.DOMA4171, fieldElement,
                    sequenceGeneratorReflection.getAnnotationMirror(),
                    sequenceGeneratorReflection.getImplementer(),
                    new Object[] { typeElement.getQualifiedName() });
        }
    ...
    private void doTableIdGeneratorMeta(EntityPropertyMeta propertyMeta) {
        TableGeneratorReflection tableGeneratorReflection = ctx.getReflections()
    ...
    private void validateTableIdGenerator(EntityPropertyMeta propertyMeta,
        ...
        if (typeElement.getModifiers().contains(Modifier.ABSTRACT)) {
            throw new AptException(Message.DOMA4168, fieldElement,
        ...
        ExecutableElement constructor = ctx.getElements().getNoArgConstructor(typeElement);
        if (constructor == null || !constructor.getModifiers().contains(Modifier.PUBLIC)) {
            throw new AptException(Message.DOMA4169, fieldElement,
                    tableGeneratorReflection.getAnnotationMirror(),
                    tableGeneratorReflection.getImplementer(),
                    new Object[] { typeElement.getQualifiedName() });
        }
    ...
    private void doColumn(EntityPropertyMeta propertyMeta) {
        ...
        if (propertyMeta.isId() || propertyMeta.isVersion()) {
            if (!columnReflection.getInsertableValue()) {
                throw new AptException(Message.DOMA4088, fieldElement,
                        columnReflection.getAnnotationMirror(), columnReflection.getInsertable());
            }
            if (!columnReflection.getUpdatableValue()) {
                throw new AptException(Message.DOMA4089, fieldElement,
                        columnReflection.getAnnotationMirror(), columnReflection.getUpdatable());
            }
        }
    }

    private boolean isNumber(CtType ctType) {
        Boolean isNumber = ctType
