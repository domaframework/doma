 ...
 * 
 */
public class ExternalHolderMetaFactory implements
        TypeElementMetaFactory<ExternalHolderMeta> {

    private final Context ctx;

    public ExternalHolderMetaFactory(Context ctx) {
        assertNotNull(ctx);
        this.ctx = ctx;
    }

    @Override
    public ExternalHolderMeta createTypeElementMeta(TypeElement convElement) {
    ...
    private void validateConverter(TypeElement convElement) {
        ...
        ExecutableElement constructor = ctx.getElements()
                .getNoArgConstructor(convElement);
        if (constructor == null
                || !constructor.getModifiers().contains(Modifier.PUBLIC)) {
            throw new AptException(Message.DOMA4193, convElement, new Object[] { convElement.getQualifiedName() });
        }
    }
    ...
    private void validateEnclosingElement(Element element) {
        ...
        if (simpleName.contains(Constants.BINARY_NAME_DELIMITER)
            ...
            throw new AptException(Message.DOMA4280, typeElement, new Object[] { typeElement.getQualifiedName() });
        }
        ...
        if (nestingKind == NestingKind.TOP_LEVEL) {
            ...
            if (modifiers.containsAll(Arrays.asList(Modifier.STATIC,
                ...
                throw new AptException(Message.DOMA4278, typeElement, new Object[] { typeElement.getQualifiedName() });
            }
        } else {
            throw new AptException(Message.DOMA4279, typeElement, new Object[] { typeElement.getQualifiedName() });
        }
    }
