...
public abstract class AbstractSqlFileQueryMetaFactory<M extends AbstractSqlFileQueryMeta>
    ...
    protected void doSqlFiles(M queryMeta, boolean expandable, boolean populatable) {
        ...
        String filePath = SqlFileUtil.buildPath(getDaoElement().getQualifiedName().toString(),
                queryMeta.getName());
        ...
        for (File siblingfile : siblingfiles) {
            if (SqlFileUtil.isSqlFile(siblingfile, methodName)) {
                ...
                if (sql.isEmpty() || StringUtil.isWhitespace(sql)) {
                    throw new AptException(Message.DOMA4020, methodElement,
                            new Object[] { sqlFilePath });
                }
                SqlNode sqlNode = createSqlNode(queryMeta, sqlFilePath, sql);
                SqlValidator validator = createSqlValidator(queryMeta.getBindableParameterTypeMap(),
    ...
    protected File getFile(M queryMeta, String filePath) {
        ...
        if (!file.exists()) {
            throw new AptException(Message.DOMA4019, methodElement,
                    new Object[] { filePath, file.getAbsolutePath() });
        }
        if (file.isDirectory()) {
            throw new AptException(Message.DOMA4021, methodElement,
                    new Object[] { filePath, file.getAbsolutePath() });
        }
        if (!IOUtil.endsWith(file, filePath)) {
            throw new AptException(Message.DOMA4309, methodElement,
                    new Object[] { filePath, file.getAbsolutePath() });
        }
        return file;
    }
    ...
    protected File[] getSiblingFiles(M queryMeta, File file) {
        ...
        if (files == null) {
            throw new AptException(Message.DOMA4144, methodElement,
    ...
    protected String getSql(File file, String filePath) {
        try {
            ...
            throw new AptException(Message.DOMA4068, methodElement, cause,
    ...
    protected SqlValidator createSqlValidator(LinkedHashMap<String, TypeMirror> parameterTypeMap,
        ...
        return new SqlValidator(ctx, methodElement, parameterTypeMap, sqlFilePath, expandable,
                populatable);
    }
}
