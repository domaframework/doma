...
public class ExpressionEvaluator implements ExpressionNodeVisitor<EvaluationResult, Void> {
  ...
  public EvaluationResult visitEqOperatorNode(EqOperatorNode node, Void p) {
    var left = node.getLeftNode().accept(this, p).getValue();
    var right = node.getRightNode().accept(this, p).getValue();
    ...
    try {
      ...
      var c1 = (Comparable<Object>) left;
      @SuppressWarnings("unchecked")
      var c2 = (Comparable<Object>) right;
  ...
  public EvaluationResult visitNotOperatorNode(NotOperatorNode node, Void p) {
    var result = node.getNode().accept(this, p).getBooleanValue();
    return new EvaluationResult(!result, boolean.class);
  }
  ...
  public EvaluationResult visitAddOperatorNode(AddOperatorNode node, Void p) {
    ...
    var leftResult = evaluateNotNullableOperandNode(node, leftNode, p);
    var rightNode = node.getRightNode();
    var rightResult = evaluateNotNullableOperandNode(node, rightNode, p);

    var leftText = createText(node, leftNode, leftResult);
    if (leftText != null) {
      var rightText = createText(node, rightNode, rightResult);
      if (rightText != null) {
    ...
    var leftNumber = createNumber(node, leftNode, leftResult);
    if (leftNumber == null) {
      throw newNonNumberException(node, leftNode, leftResult);
    }
    var rightNumber = createNumber(node, rightNode, rightResult);
    if (rightNumber == null) {
  ...
  protected void throwNotTextException(
      OperatorNode operatorNode, ExpressionNode operandNode, EvaluationResult evaluationResult) {
    var location = operandNode.getLocation();
    throw new ExpressionException(
        Message.DOMA3020,
        location.getExpression(),
        location.getPosition(),
  ...
  protected ExpressionException newNonNumberException(
      ArithmeticOperatorNode operatorNode,
      ExpressionNode operandNode,
      EvaluationResult evaluationResult) {
    var location = operandNode.getLocation();
  ...
  protected EvaluationResult evaluateNotNullableOperandNode(
    ...
    var evaluationResult = operandNode.accept(this, p);
  ...
  public EvaluationResult visitNewOperatorNode(NewOperatorNode node, Void p) {
    ...
    var collection = collector.collect(node.getParametersNode());
    var location = node.getLocation();
    var className = node.getClassName();
    var clazz = forClassName(location, className);
    var constructor = findConstructor(location, clazz, collection.getParamTypes());
    if (constructor == null) {
      var signature = ConstructorUtil.createSignature(clazz, collection.getParamTypes());
      throw new ExpressionException(
  ...
  public EvaluationResult visitMethodOperatorNode(MethodOperatorNode node, Void p) {
    var targetObjectNode = node.getTargetObjectNode();
    var targetResult = targetObjectNode.accept(this, p);
    var target = targetResult.getValue();
    if (target == null) {
      var location = node.getLocation();
      throw new ExpressionException(
          Message.DOMA3027,
    ...
    var collection = collector.collect(node.getParametersNode());
    var location = node.getLocation();
    var method = findMethod(node.getMethodName(), target, targetClass, collection.getParamTypes());
    if (method == null) {
      var signature = MethodUtil.createSignature(node.getMethodName(), collection.getParamTypes());
      throw new ExpressionException(
          Message.DOMA3002,
          location.getExpression(),
          location.getPosition(),
  ...
  public EvaluationResult visitStaticMethodOperatorNode(StaticMethodOperatorNode node, Void p) {
    var targetClass = forClassName(node.getLocation(), node.getClassName());
    var collector = new ParameterCollector();
    var collection = collector.collect(node.getParametersNode());
    var location = node.getLocation();
    var method = findMethod(node.getMethodName(), null, targetClass, collection.getParamTypes());
    if (method == null) {
      var signature = MethodUtil.createSignature(node.getMethodName(), collection.getParamTypes());
      throw new ExpressionException(
          Message.DOMA3002,
  ...
  protected Method findMethod(
    ...
    var result = findMethodFromInterfaces(methodName, target, targetClass, paramTypes);
    if (result != null) {
      return result;
    }
  ...
  protected Method findSuiteMethod(
    ...
    for (var method : methods) {
      ...
      if (paramTypes.length == argTypes.length) {
        var degreeOfCoincidence = 0;
        for (var i = 0; i < paramTypes.length; i++) {
          var difference = calculateHierarchyDifference(paramTypes[i], argTypes[i], 0);
          if (difference == -1) {
            continue outer;
          }
  ...
  protected int calculateHierarchyDifference(
    ...
    for (var type = argType; type != null; type = type.getSuperclass()) {
      ...
      if (paramType.isInterface()) {
        for (var interfaceClass : type.getInterfaces()) {
          var result = calculateHierarchyDifference(paramType, interfaceClass, difference);
          if (result != -1) {
            return result;
          }
        }
  ...
  protected Method findStaticMethod(
    ...
    var method = findMethod(methodName, null, targetClass, paramTypes);
    if (method == null) {
      return null;
    }
  ...
  public EvaluationResult visitFieldOperatorNode(FieldOperatorNode node, Void p) {
    var targetResult = node.getTargetObjectNode().accept(this, p);
    var target = targetResult.getValue();
    var location = node.getLocation();
    var field = findField(node.getFieldName(), target.getClass());
    if (field == null) {
      throw new ExpressionException(
          Message.DOMA3018,
          location.getExpression(),
  ...
  public EvaluationResult visitStaticFieldOperatorNode(StaticFieldOperatorNode node, Void p) {
    var targetClass = forClassName(node.getLocation(), node.getClassName());
    var location = node.getLocation();
    var field = findStaticField(node.getFieldName(), targetClass);
    if (field == null) {
      throw new ExpressionException(
          Message.DOMA3033,
          location.getExpression(),
  ...
  protected Field findField(String fieldName, Class<?> targetClass) {
    for (var clazz = targetClass; clazz != Object.class; clazz = clazz.getSuperclass()) {
      try {
  ...
  protected Field findStaticField(String fieldName, Class<?> targetClass) {
    var field = findField(fieldName, targetClass);
  ...
  public EvaluationResult visitVariableNode(VariableNode node, Void p) {
    ...
    var value = variableValues.get(node.getExpression());
    if (value == null) {
      var location = node.getLocation();
      throw new ExpressionException(
  ...
  protected static class Text {
    ...
    protected EvaluationResult concat(Text other) {
      var newValue = stringValue.concat(other.stringValue);
  ...
  protected static class Number {
    ...
    protected void handleArithmeticException(ArithmeticException e) {
      var location = operatorNode.getLocation();
      throw new ExpressionException(
          Message.DOMA3014,
          e,
          location.getExpression(),
    ...
    protected EvaluationResult createEvaluationResult(Number other, BigDecimal newValue) {
      var realClass = this.priority >= other.priority ? this.realClass : other.realClass;
      var narrowedValue = narrowValue(newValue, realClass);
  ...
  protected class ParameterCollector
    ...
    protected void evaluate(ExpressionNode node, List<EvaluationResult> p) {
      var evaluationResult = ExpressionEvaluator.this.evaluateInternal(node);
  ...
  protected static class ParameterCollection {
    ...
    public ParameterCollection(List<EvaluationResult> evaluationResults) {
      ...
      var size = evaluationResults.size();
