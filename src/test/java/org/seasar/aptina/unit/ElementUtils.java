...
class ElementUtils {
   ...
   *
   * <p>コンパイルのコンテキスト外でこのメソッドが返す {@link TypeElement} およびその {@link Element#getEnclosedElements()} が返す
   * {@link Element} から， {@link Elements#getDocComment(Element)} を使って Javadoc コメントを取得することはできません．
   *
   * @param elementUtils {@link Elements}
   * @param clazz クラス
   * @return クラスに対応する{@link TypeElement}， 存在しない場合は {@literal null}
   */
  public static TypeElement getTypeElement(final Elements elementUtils, final Class<?> clazz) {
   ...
   * @return クラスに対応する{@link TypeElement}， 存在しない場合は {@literal null}
   */
  public static TypeElement getTypeElement(final Elements elementUtils, final String className) {
   ...
   */
  public static VariableElement getFieldElement(final TypeElement typeElement, final Field field) {
   ...
   */
  public static ExecutableElement getConstructorElement(final TypeElement typeElement) {
    assertNotNull("typeElement", typeElement);
  ...
  public static ExecutableElement getConstructorElement(
    ...
    for (final ExecutableElement executableElement :
        ElementFilter.constructorsIn(typeElement.getEnclosedElements())) {
      if (isSameTypes(parameterTypeNames, executableElement.getParameters())) {
        return executableElement;
      }
    }
    return null;
  ...
  public static ExecutableElement getMethodElement(
    ...
    for (final ExecutableElement executableElement :
      ...
      if (!methodName.equals(executableElement.getSimpleName().toString())) {
        continue;
      }
      if (executableElement.getParameters().size() == 0) {
        return executableElement;
   ...
   *
   * @param element 注釈の付けられた {@link Element}
   ...
   *
   * @param element 注釈の付けられた {@link Element}
   * @param annotationClassName アノテーションのクラス名
   * @return {@link Element} に付けられた指定の {@link AnnotationMirror}
   */
  public static AnnotationMirror getAnnotationMirror(
      final Element element, final String annotationClassName) {
   ...
   *
   * @param parameterTypes 引数型の配列
   * @param variableElements
   * @return 二つのリストのそれぞれの要素の型がマッチすれば {@code true}
   */
  public static boolean isSameTypes(
      final Class<?>[] parameterTypes, final List<? extends VariableElement> variableElements) {
    assertNotNull("parameterTypes", parameterTypes);
    assertNotNull("variableElements", variableElements);
    return isSameTypes(getQualifiedNameArray(parameterTypes), variableElements);
  }
   ...
   */
  public static boolean isSameTypes(
      final String[] typeNames, final List<? extends VariableElement> variableElements) {
    ...
    for (int i = 0; i < typeNames.length; ++i) {
      if (!typeNames[i].equals(variableElements.get(i).asType().toString())) {
        return false;
      }
    }
    return true;
   ...
   *
   * @param elements {@link Element} の {@link List}
   * @return {@link Element#getSimpleName()} が返す名前の {@link List}
   */
  public static List<String> toSimpleNameList(final List<? extends Element> elements) {
   ...
   * 型引数宣言の完全な文字列表現を返します．
   *
   * @param typeParameters 型引数の {@link List}
  ...
  public static String toStringOfTypeParameterDecl(
    ...
    for (final TypeParameterElement typeParameter : typeParameters) {
      ...
      if (bounds.size() > 1 || bounds.get(0).toString() == "java.lang.Object") {
        buf.append(" extends ").append(join(toTypeNameList(bounds), " & "));
      }
      parameterPrefix = ", ";
    }
    buf.append('>');
  ...
  public static String toStringOfTypeParameterNames(
    ...
    final StringBuilder buf = new StringBuilder(64);
    buf.append("<").append(join(toSimpleNameList(typeParameters), ", ")).append(">");
    return new String(buf);
  }
}
