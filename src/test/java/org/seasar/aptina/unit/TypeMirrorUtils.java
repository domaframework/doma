...
class TypeMirrorUtils {
  ...
  public static TypeMirror getTypeMirror(
      final Types typeUtils, final Elements elementUtils, final Class<?> clazz) {
    assertNotNull("typeUtils", typeUtils);
    assertNotNull("elementUtils", elementUtils);
    assertNotNull("clazz", clazz);
    if (clazz.isArray()) {
      return toArrayType(
          typeUtils, getTypeMirror(typeUtils, elementUtils, clazz.getComponentType()));
    }
   ...
   */
  public static TypeMirror getTypeMirror(
      final Types typeUtils, final Elements elementUtils, final String className) {
    assertNotNull("typeUtils", typeUtils);
    assertNotNull("elementUtils", elementUtils);
    assertNotEmpty("className", className);
    if (className.endsWith("[]")) {
      final String componentTypeName = className.substring(0, className.length() - 2);
      return toArrayType(typeUtils, getTypeMirror(typeUtils, elementUtils, componentTypeName));
    }
    if (className.startsWith("[") && className.endsWith(";")) {
      final int pos = className.indexOf("L");
      final String componentTypeName = className.substring(pos + 1, className.length() - 1);
      TypeMirror typeMirror = getTypeMirror(typeUtils, elementUtils, componentTypeName);
      for (int i = 0; i < pos; ++i) {
   ...
   *
   * @param typeUtils {@link Types}
   * @param componentTypeMirror 配列の要素となる型
   * @return 引数の型を要素とする配列の {@link TypeMirror}
   * @see Types#getArrayType(TypeMirror)
   */
  public static TypeMirror toArrayType(
      final Types typeUtils, final TypeMirror componentTypeMirror) {
   ...
   * @throws IllegalArgumentException 配列の要素のクラスに対応する {@link TypeMirror} が存在しない場合
   */
  public static List<TypeMirror> toTypeMirrors(
      final Types typeUtils, final Elements elementUtils, final Class<?>... types)
    ...
    for (final Class<?> type : types) {
      final TypeMirror typeMirror = getTypeMirror(typeUtils, elementUtils, type);
      if (typeMirror == null) {
   ...
   * @param typeUtils {@link Types}
   * @param elementUtils {@link Elements}
   * @param typeNames クラス名の配列
   * @return {@link TypeMirror} の配列
   * @throws IllegalArgumentException 配列の要素のクラスに対応する {@link TypeMirror} が存在しない場合
   */
  public static List<TypeMirror> toTypeMirrors(
      final Types typeUtils, final Elements elementUtils, final String... typeNames) {
    assertNotNull("typeUtils", typeUtils);
    assertNotNull("elementUtils", elementUtils);
    assertNotNull("typeNames", typeNames);
    final List<TypeMirror> typeMirrors = newArrayList();
    for (final String typeName : typeNames) {
      final TypeMirror typeMirror = getTypeMirror(typeUtils, elementUtils, typeName);
      if (typeMirror == null) {
   ...
   *
   * @param typeMirrors {@link TypeMirror} の {@link List}
   * @return {@link TypeMirror} の文字列表現の {@link List}
   */
  public static List<String> toTypeNameList(final List<? extends TypeMirror> typeMirrors) {
