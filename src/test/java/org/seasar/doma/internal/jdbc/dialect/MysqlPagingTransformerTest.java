...
public class MysqlPagingTransformerTest extends TestCase {
  ...
  public void testOffsetLimit() throws Exception {
    var expected = "select * from emp order by emp.id limit 5, 10";
    var transformer = new MysqlPagingTransformer(5, 10);
  ...
  public void testOffsetLimit_forUpdate() throws Exception {
    var expected = "select * from emp order by emp.id  limit 5, 10 for update";
    var transformer = new MysqlPagingTransformer(5, 10);
  ...
  public void testOffsetOnly() throws Exception {
    var expected = "select * from emp order by emp.id limit 5, 18446744073709551615";
    var transformer = new MysqlPagingTransformer(5, -1);
    var parser = new SqlParser("select * from emp order by emp.id");
    var sqlNode = transformer.transform(parser.parse());
    var sqlBuilder = new NodePreparedSqlBuilder(new MockConfig(), SqlKind.SELECT, "dummyPath");
    var sql = sqlBuilder.build(sqlNode, Function.identity());
  ...
  public void testLimitOnly() throws Exception {
    var expected = "select * from emp order by emp.id limit 0, 10";
    var transformer = new MysqlPagingTransformer(-1, 10);
    var parser = new SqlParser("select * from emp order by emp.id");
    var sqlNode = transformer.transform(parser.parse());
    var sqlBuilder = new NodePreparedSqlBuilder(new MockConfig(), SqlKind.SELECT, "dummyPath");
    var sql = sqlBuilder.build(sqlNode, Function.identity());
