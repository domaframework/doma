...
public abstract class AptinaTestCase extends TestCase {
    ...
    protected final List<String> options = newArrayList();

    protected final List<File> sourcePaths = newArrayList();

    protected final List<File> classOutputPaths = newArrayList();

    protected final List<Processor> processors = newArrayList();
    {
        processors.add(new AptinaUnitProcessor());
    }

    protected final List<CompilationUnit> compilationUnits = newArrayList();

    protected JavaCompiler javaCompiler;

    protected DiagnosticCollector<JavaFileObject> diagnostics;

    protected StandardJavaFileManager standardJavaFileManager;

    protected JavaFileManager testingJavaFileManager;

    protected ProcessingEnvironment processingEnvironment;

    protected Boolean compiledResult;
    ...
    protected void addClassOutputPath(final File... classOutputPaths) {
        assertNotEmpty("classOutputPath", classOutputPaths);
        this.classOutputPaths.addAll(asList(classOutputPaths));
    }

    protected void addClassOutputPath(final String... classOutputPaths) {
        assertNotEmpty("classOutputPath", classOutputPaths);
        for (final String path : classOutputPaths) {
            this.classOutputPaths.add(new File(path));
        }
    }
