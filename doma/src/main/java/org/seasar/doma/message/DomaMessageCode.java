...
public enum DomaMessageCode implements MessageCode {
    ...
    DOMA4092("SQL[{0}]の検証処理に失敗しました（[{1}]行目[{2}]番目の文字付近）。原因は次のものです。{3}"),
    ...
    // bean
    DOMA6001("クラス[{0}]のプロパティ[{1}]へ値を設定できませんでした。原因は次のものです。{2}"),
    DOMA6002("クラス[{0}]のプロパティ[{1}]から値を取得できませんでした。原因は次のものです。{2}"),
    DOMA6003("イントロスペクトション中に例外が発生しました。原因は次のものです。{0}"),

    // copy
    DOMA7001("クラス[{0}]のプロパティ[{1}]の値[{2}]をコピーできませんでした。原因は次のものです。{3}"),

    // entity
    DOMA8001("パラメータ[{0}]の値[{1}]の型がEntityインタフェースの実装クラスではありません。"),
