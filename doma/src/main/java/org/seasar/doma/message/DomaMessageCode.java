...
public enum DomaMessageCode implements MessageCode {
    ...
    DOMA3017("式[{0}]の評価に失敗しました（[{1}]番目の文字付近）。プロパティ[{2}]に対応するオブジェクトを解決できませんでした。変数名が正しいか確認してください。"),
    ...
    DOMA4041("パラメータのList型には型パラメータが必須です。"),
    DOMA4042("パラメータはList型のサブタイプでなければいけません。"),
    ...
    DOMA4050("@MappedSuperclassが注釈されたクラスは型パラメータを受け取ってはいけません。"),
    ...
    DOMA4061("List型の型パラメータ[{0}]はサポートされていません。"),
    ...
    DOMA4067("式[{0}]（[{1}]番目の文字付近）に含まれる変数[{2}]に対応するパラメータが存在しません。"),
    ...
    DOMA4072("式[{0}]（[{1}]番目の文字付近）に含まれる変数[{2}]（フィールドもしくはメソッドの戻り値の型が[{3}]）が解決できません。コンパイルに失敗していませんか。"),
    ...
    DOMA4092("SQL[{0}]の検証処理に失敗しました（[{1}]行目[{2}]番目の文字付近）。詳細は次のものです。{3}"),
