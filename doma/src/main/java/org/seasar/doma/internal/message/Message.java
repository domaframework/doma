...
public enum Message implements MessageResource {
    ...
    DOMA4162("ユーザー定義の設定クラスは指定必須です（ただし、例外として@AnnotateWithを併用する場合は指定不要です。この場合、org.seasar.doma.jdbc.ConfigProxyが指定されているとみなされます。）。ユーザー定義の設定クラスは、rg.seasar.doma.jdbc.Configのインスタンス化可能なサブタイプでなければいけません。"),
