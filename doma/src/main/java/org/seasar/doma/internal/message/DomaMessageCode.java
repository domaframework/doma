...
public enum DomaMessageCode implements MessageCode {
    ...
    DOMA2002("カラム[{0}]が結果セットに含まれますが、このカラムにマッピングされたプロパティがエンティティクラス[{2}]に見つかりません。エンティティクラスに[{1}]という名前のプロパティを定義するか、任意のプロパティに@Columnを注釈し@Columnのname要素に[{0}]を指定してください。SQLファイルパス=[{2}]。ログ用SQL=[{3}]"),
