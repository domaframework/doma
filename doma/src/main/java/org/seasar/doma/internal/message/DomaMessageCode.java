...
public enum DomaMessageCode implements MessageCode {
    ...
    DOMA2002("カラム[{0}]が結果セットに含まれますが、このカラムにマッピングされたプロパティがエンティティクラス[{2}]に見つかりません。エンティティクラスに[{1}]という名前のプロパティを定義するか、任意のプロパティに@Columnを注釈し@Columnのname要素に[{0}]を指定してください。SQLファイルパス=[{3}]。ログ用SQL=[{4}]"),
    ...
    DOMA2131("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/の式[{3}]に対応するオブジェクトの型[{4}]がString、Character、charのいずれでもありません。"),
