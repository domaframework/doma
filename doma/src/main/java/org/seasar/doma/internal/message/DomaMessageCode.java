...
public enum DomaMessageCode implements MessageCode {
    ...
    DOMA2002("カラム[{0}]にマッピングされたプロパティがエンティティクラス[{1}]に見つかりません。SQLファイルパス=[{2}]。ログ用SQL=[{3}]"),
    ...
    DOMA2016("PreparedStatementの取得に失敗しました。SQLファイルパス=[{0}]。実際のSQL=[{1}]。原因は次のものです。{2}"),
    ...
    DOMA2025("CallableStatementの取得に失敗しました。実際のSQL=[{0}]。原因は次のものです。{1}"),
    ...
    DOMA2127("SQL[{0}]の解析に失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/に対応する/*%for ...*/が見つかりません。"),
    DOMA2128("SQL[{0}]の解析に失敗しました（[{1}]行目[{2}]番目の文字付近）。複数の/*hasNext ...*/が存在します。"),
    DOMA2129("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%for ...*/の中のトークン[:]の後に続く式[{3}]に対応するオブジェクトの型[{4}]がIterableインタフェースのサブタイプではありません。"),
    DOMA2131("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/の式[{3}]に対応するオブジェクトの型[{4}]がStringまたはchar/Characterではありません。"),
    DOMA2132("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。埋め込み変数コメント[{3}]に対応するオブジェクトの型[{4}]がStringまたはchar/Characterではありません。"),
    DOMA2133("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/の式[{3}]の結果にシングルクォテーションが含まれています。"),
    DOMA2134("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/の式[{3}]の結果にセミコロンが含まれています。"),
    DOMA2135("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/の式[{3}]の結果に行コメントが含まれています。"),
    DOMA2136("SQL[{0}]の組み立てに失敗しました（[{1}]行目[{2}]番目の文字付近）。/*%hasNext ...*/の式[{3}]の結果にブロックコメントが含まれています。"),
    ...
    DOMA4134("エンティティクラスはabstractであってはいけません。"),
