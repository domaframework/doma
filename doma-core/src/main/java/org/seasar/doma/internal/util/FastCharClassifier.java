...
/**
 * High-performance character classification using pre-computed lookup tables.
 *
 * <p>This class provides O(1) character classification for ASCII characters by using pre-computed
 * byte arrays. For non-ASCII characters, it falls back to standard Java methods.
 *
 ...
 * </ul>
 *
 * @see SqlTokenUtil for the previous implementation
 */
public final class FastCharClassifier {
  ...
  /**
   * Gets the character type flags for debugging and testing purposes.
   *
   * @param c the character to get type flags for
   * @return the bit flags for the character, or -1 for non-ASCII characters
   */
  public static int getCharTypeFlags(char c) {
    return c < ASCII_SIZE ? CHAR_TYPES[c] : -1;
  }
}
