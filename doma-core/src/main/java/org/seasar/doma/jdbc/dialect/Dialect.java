...
public interface Dialect {
  ...
  /**
   * Normalizes a column name for use with auto-generated key retrieval via {@link
   * java.sql.Connection#prepareStatement(String, String[])}.
   *
   * <p>The default implementation returns the column name as-is. Dialects whose drivers require a
   * specific case (e.g. PostgreSQL folds unquoted identifiers to lower-case) should override this
   * method.
   *
   * @param columnName the column name to normalize (without quote characters)
   * @param quoteRequired whether the column requires quoting
   * @return the normalized column name, optionally surrounded by quote characters
   */
  default String normalizeColumnNameForGeneratedKeys(String columnName, boolean quoteRequired) {
    return columnName;
  }
