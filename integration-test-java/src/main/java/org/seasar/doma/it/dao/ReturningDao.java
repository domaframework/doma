...
public interface ReturningDao {
  ...
  Employee insertThenReturnAll(Employee employee);

  @Insert(returning = @Returning, duplicateKeyType = DuplicateKeyType.IGNORE)
  Optional<Employee> insertOrIgnoreThenReturnAll(Employee employee);

  @Insert(returning = @Returning)
  Employee insertThenReturnOnlyId(Employee employee);

  @Insert(returning = @Returning)
  Employee insertThenReturnExceptVersion(Employee employee);
