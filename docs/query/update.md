...
```{note}
  ...
  When updating via SQL file, the rules differ depending on whether you use [\1](\1).
...
The rules for controlling update target properties are the same as in [\1](\1).
...
If the `queryTimeout` property is not set, the query timeout specified in [\1](\1) is used.
