...
テーブル名は :ref:`naming-label` により解決されます。
...
フィールドの型は、 :doc:`basic` もしくは :doc:`domain` でなければいけません。
...
カラム名は :ref:`naming-label` により解決されます。
...
* ``java.lang.Number`` のサブタイプを要素とする ``java.util.Optional``
