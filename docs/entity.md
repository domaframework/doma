...
```java
  ...
  In this case, the entity is recognized as [\1](\1)
...
If the `@Table` annotation is not specified, the table name is determined by the [\1](\1) .
...
If the `@Column` annotation is not specified, the column name is determined by the [\1](\1) .

```{note}

  When the field type is [\1](\1), you cannot apply the `@Column` annotation to the field.
...
```{note}
  ...
  When the field type is [\1](\1), you cannot apply the `@Id` annotation to the field.
...
* [\1](\1), whose value type is the subclass of java.lang.Number
...
```{note}
  ...
  When the field type is [\1](\1), you cannot apply the `@Version` annotation to the field.
...
```{note}
  ...
  When the field type is [\1](\1), you cannot apply the `@TenantId` annotation to the field.
